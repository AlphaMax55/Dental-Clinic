<?php 
// ============================================================
// ========== SLIDER - VERİTABANINDAN ÇEK ==========
// ============================================================

$lang = $_SESSION['lang'] ?? 'tr';

try {
    $stmt = $db->prepare("SELECT * FROM anasayfa_slider WHERE aktif = 1 ORDER BY sira ASC");
    $stmt->execute();
...
// FALLBACK: Veritabanı boşsa manuel array
if (empty($slider_data)) {
    ...
    foreach ($slider_data as $row) {
        ...
        // ✅ t_cevir() ile sar (çoklu dil desteği için)
        $hero_slides[] = [
            'title' => t_cevir($title),
...
<section class="relative w-full md:h-[400px] lg:h-[460px] overflow-hidden bg-slate-900" id="heroSlider" style="height:410px">
...
<div class="absolute inset-0">
    <img src="<?php echo $slide['image']; ?>" 
         ...
         fetchpriority="high"
         loading="eager">
    <div class="absolute inset-0 bg-gradient-to-r from-slate-900/80 via-slate-900/40 to-slate-900/20"></div>
</div>
        <div class="relative z-20 h-full max-w-[1400px] mx-auto px-6 lg:px-12 flex items-center">
            <div class="max-w-2xl">
                ...
                <div class="flex flex-wrap gap-3">
                    <a class="group flex items-center gap-2 bg-gradient-to-r from-[#007aff] to-blue-600 text-white px-6 py-3 rounded-xl font-semibold text-sm hover:shadow-lg hover:-translate-y-0.5 transition-all" href="<?php echo SITE_PATH; ?>/iletisim/"><?php echo t_cevir("İletişime Geç"); ?><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-arrow-right group-hover:translate-x-1 transition-transform"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg></a>
                    <a class="group flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 text-white px-6 py-3 rounded-xl font-semibold text-sm hover:bg-white/20 transition-all" href="<?php echo SITE_PATH; ?>/tedaviler/"><?php echo t_cevir("Tedaviler"); ?></a>
                </div>
            </div>
        </div>
    </div>
    ...
    <button id="heroPrevBtn" aria-label="Önceki Slayt" class="absolute left-4 top-1/2 -translate-y-1/2 z-40 w-10 h-10 rounded-full bg-white/10 hover:bg-cyan-500 text-white transition-all backdrop-blur-sm border border-white/20 hidden md:flex items-center justify-center">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-chevron-left"><path d="m15 18-6-6 6-6"></path></svg>
    </button>
    <button id="heroNextBtn" aria-label="Sonraki Slayt" class="absolute right-4 top-1/2 -translate-y-1/2 z-40 w-10 h-10 rounded-full bg-white/10 hover:bg-cyan-500 text-white transition-all backdrop-blur-sm border border-white/20 hidden md:flex items-center justify-center">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-chevron-right"><path d="m9 18 6-6-6-6"></path></svg>
    </button>
    ...
    <div id="stickyRandevuBtn" class="fixed bottom-0 left-0 right-0 p-3 z-[100] md:hidden hidden animate-in fade-in slide-in-from-bottom-5 duration-300">
        <a href="<?php echo SITE_PATH; ?>/iletisim/" class="flex items-center justify-center gap-2 bg-gradient-to-r from-[#007aff] to-blue-600 text-white w-full py-3.5 rounded-xl font-semibold text-sm shadow-lg active:scale-95 transition-all">
            <?php echo t_cevir("Hemen İletişime Geçin"); ?> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-arrow-right"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
        </a>
    </div>
</section>
...
function showSlide(index) {
    ...
    // ✅ Tek requestAnimationFrame'de tüm değişiklikleri yap
    requestAnimationFrame(() => {
        // Eski slide'ı pasif yap
        ...
        // Transition bitince flag'i sıfırla
        setTimeout(() => {
            isTransitioning = false;
        }, 600); // Transition süresi kadar
    });
}   
    ...
    // Sayfa görünürlük kontrolü (performance)
    ...
    // Scroll ile sticky button (throttle ile)
...
function init() {
    ...
    // ✅ CSS'i sadece bir kere ekle (varsa tekrar ekleme)
    if (!document.getElementById('hero-slider-styles')) {
        ...
        style.textContent = `
            .hero-slide {
                ...
                /* will-change KALDIRILDI - gereksiz GPU katmanı */
                /* backface-visibility KALDIRILDI - gereksiz */
            }
    ...
    // ✅ Buton event listener'ları (passive true ile)
    if (prevBtn) {
    ...
    // ✅ Visibility API (passive ile)
    document.addEventListener('visibilitychange', handleVisibilityChange);
    
    // ✅ Preload'u requestIdleCallback ile yap
...
<script>
// ========== OPTİMİZE EDİLMİŞ TEDAVİ SLIDER (FINAL) ==========
(function() {
    'use strict';
    ...
    // Slide güncelleme (en performanslı versiyon)
    ...
    // Dot'ları güncelle (sadece class değişimi, innerHTML yok)
    function updateDots() {
    ...
    // Navigasyon butonlarını kontrol et (varsa)
    ...
    // Dot'ları oluştur (sadece bir kere)
    function createDots() {
    ...
    // Slider'ı sıfırla (sayfa yenileme gibi durumlar için)
    window.resetTreatmentSlider = function() {
        currentSlide = 0;
        updateTreatmentGrid();
        resetTimer();
    };
    
    // Global navigasyon fonksiyonları
    window.nextTreatmentSlide = nextSlide;
    window.prevTreatmentSlide = prevSlide;
    window.goToTreatmentSlide = goToSlide;
    
    // Mouse ile hover'da timer duraklat (opsiyonel)
    ...
    function init() {
        ...
        // Sayfa gizlenince timer'ı durdur (performance)
